...
xcselect_manpaths *xcselect_get_manpaths(char *sdkname)
{
	...
	if ((_xpc_runtime_is_app_sandboxed() & 1) == 0 && (xcselect_get_developer_dir_path(path, MAXPATHLEN, &ignored, &ignored, &ignored)) != 0) {
		...
		if (path_exists(path_xcrun) != 0) {
			...
			*((void**)&xcrun_iter_manpaths) = dlsym(open_xcrun, "xcrun_iter_manpaths");
